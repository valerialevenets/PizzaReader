...
docker exec dev-backend-1 ./dev/wait-for-it.sh dev-db-1:3306 -t 6000

# It can return non-zero code - just ignore
set +e
docker exec dev-backend-1 chmod -R -f 777 data
set -e

docker exec dev-backend-1 composer install
docker exec dev-worker-1 composer install
